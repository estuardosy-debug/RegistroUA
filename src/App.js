...
import { 
  ...
  Settings, Search, Bell, Menu, X, ChevronDown, 
  Copy, Download, UserPlus, Shield, Gavel, LayoutDashboard 
} from 'lucide-react';
...
// Asegúrate de que esto funcione en tu entorno local/Vercel.
// Si usas CodeSandbox, recuerda reemplazar esto con tu config real si falla la variable.
...
const PublicRegistration = ({ onViewChange, isConnected }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: '',
    email: '',
    causaCode: '', // Ahora vacío para obligar selección
    causaCodeCustom: '', // Nuevo campo para código personalizado
  ...
  // Generar lista de años (Actual hasta 1988)
  ...
  // Cargar configuración global (Listas dinámicas)
  useEffect(() => {
    ...
    const unsub = onSnapshot(settingsRef, (docSnap) => {
      ...
      console.log("Info: Configuración personalizada no encontrada, usando defaults.");
    });
  ...
  // Manejador especial para rellenar ceros al perder el foco (onBlur)
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isFormValid() || !isConnected) {
      if (!isConnected) alert("Conectando con el servidor... Intente en un momento.");
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      // 2. Procesar Código de Causa
      let finalCode = formData.causaCode;
      if (formData.causaCode === 'PERSONALIZAR') {
        finalCode = padToFive(formData.causaCodeCustom); // Forzar 5 dígitos
      ...
      // Actualizar listas en Firestore si hubo cambios
      if (updateSettings) {
      ...
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'registrations'), {
        ...formData,
        causaCode: finalCode, // Guardar el valor procesado
        causaNumber: formattedNumber, // Guardar el valor procesado
              ...
              <div className="flex flex-col md:flex-row md:items-start space-y-4 md:space-y-0 md:space-x-4">
                ...
                <div className="w-full md:w-1/3">
                  ...
                  {formData.causaCode === 'PERSONALIZAR' && (
                    <input 
                      type="number" 
                      name="causaCodeCustom" 
                      value={formData.causaCodeCustom} 
                      onChange={handleInputChange}
                      onBlur={handleBlurPad}
                      placeholder="Ej: 16020"
                      className="mt-2 w-full px-3 py-2 rounded border border-[#00A8E8] font-mono text-sm animate-in fade-in"
                    />
                  )}
                </div>
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'directory'), {
          name: name,
          email: email,
        ...
        <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-6 md:px-8">
           ...
           <div className="flex-1 px-4 md:px-0">
             <h1 className="text-xl md:text-2xl font-bold text-slate-800">
               {currentView === 'registros' ? 'Audiencias del Día' : 'Gestión de Personal'}
             </h1>
           </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex items-center space-x-4 w-full md:w-auto">
                   ...
                   <div>
                     ...
                     <p className="text-2xl font-bold text-slate-800">{Object.values(groupedRegistrations).flat().length}</p>
                   </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                  ...
                  <button 
                    onClick={() => exportToCSV(registrations, `Registros_Completo_${new Date().toISOString().split('T')[0]}.csv`)}
              ...
              <div className="space-y-6">
                {Object.keys(groupedRegistrations).length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-slate-400">Intente cambiar el filtro de fecha.</p>
                  </div>
                ) : (
                  Object.entries(groupedRegistrations).map(([causa, items]) => (
                    <div key={causa} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-bottom-2">
                      <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                        <div>
                          <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">Causa</p>
                          <h3 className="text-lg font-bold text-slate-800 font-mono">{causa}</h3>
                        </div>
                        <button 
                          onClick={() => copyEmails(items.map(i => i.email), causa)}
                          className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${copiedGroup === causa ? 'bg-green-100 text-green-700' : 'bg-blue-50 text-[#00A8E8] hover:bg-blue-100'}`}
                        >
                          {copiedGroup === causa ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                          <span>{copiedGroup === causa ? 'Copiado' : 'Copiar Emails'}</span>
                        </button>
                      </div>
                            ...
                            {items.map(item => (
                              <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="px-6 py-4 space-y-1">
                                  <div className="flex items-center text-xs"><span className="w-16 text-slate-400">Tel:</span> {item.phone}</div>
                                  <div className="flex items-center text-xs"><span className="w-16 text-slate-400">Email:</span> {item.email}</div>
                                </td>
                                <td className="px-6 py-4 space-y-1">
                                  {item.fiscalia && <div className="text-xs"><span className="text-slate-400">Fiscalía:</span> {item.fiscalia}</div>}
                                  {item.locker && <div className="text-xs"><span className="text-slate-400">Casillero:</span> {item.locker}</div>}
                                  {!item.fiscalia && !item.locker && <span className="text-slate-400">-</span>}
                                </td>
            ...
            <div className="max-w-5xl mx-auto space-y-8">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden">
                <div className="bg-orange-50 px-6 py-4 border-b border-orange-100 flex items-center space-x-2">
                  ...
                  <h3 className="font-bold text-orange-900">Solicitudes Pendientes</h3>
                </div>
                <div className="divide-y divide-slate-100">
                  {pendingUsers.filter(u => u.status === 'pending').length === 0 ? (
                    <p className="p-6 text-slate-400 text-center text-sm">No hay solicitudes pendientes.</p>
                  ) : (
                    pendingUsers.filter(u => u.status === 'pending').map(u => (
                      <div key={u.id} className="p-6 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-orange-500 mt-1">Solicitado: {formatDateTime(u.createdAt)}</p>
                        </div>
                        <div className="flex space-x-3">
                          <button onClick={() => handleUserAction(u.id, 'delete')} className="text-red-500 hover:text-red-700 text-sm font-medium px-3 py-2">Rechazar</button>
                          <button onClick={() => handleUserAction(u.id, 'approve')} className="bg-[#00A8E8] hover:bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">Aprobar Acceso</button>
                        </div>
                      </div>
                    ))
                  )}
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div className="bg-slate-50 px-6 py-4 border-b border-slate-100">
                  <h3 className="font-bold text-slate-800">Auxiliares Activos</h3>
                </div>
                <table className="w-full text-left">
                  <thead className="bg-white text-slate-500 text-xs uppercase font-semibold">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-50 text-sm">
                     {pendingUsers.filter(u => u.status === 'active' || u.role === 'admin').map(u => (
                       <tr key={u.id} className="hover:bg-slate-50">
                         <td className="px-6 py-4 font-medium text-slate-800">{u.name || 'Sin nombre'}</td>
                         <td className="px-6 py-4 text-slate-600">{u.email}</td>
                         <td className="px-6 py-4">
                           <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-bold uppercase ${u.role === 'admin' ? 'bg-purple-50 text-purple-700' : 'bg-blue-50 text-blue-700'}`}>
                             {u.role}
                           </span>
                         </td>
                         <td className="px-6 py-4">
                           {u.role !== 'admin' && (
                             <button onClick={() => handleUserAction(u.id, 'delete')} className="text-red-400 hover:text-red-600 flex items-center text-xs">
...
export default function App() {
    ...
    const initAuth = async () => {
      if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
        ...
        try {
          ...
          console.log("Auth anónimo no disponible, esperando token personalizado...");
        }
      }
    };
    initAuth();
  ...
  switch (view) {
    case 'welcome':
      return <PublicRegistration onViewChange={setView} isConnected={isConnected} />;
    case 'login':
